...
5. Toggle reminder → pick time → permission if needed
6. Attach existing tags and/or create a new tag
7. Save → appears on Today if in the date window, scheduled today, and active

## C. Daily check-in

1. Open Today (optionally apply a tag filter); rows show color/icon and current streak
2. Tap habit → mark complete (undo supported); streak updates
3. Completing the habit cancels/suppresses today's reminder
4. Reminder fires at the scheduled time only if not yet completed; opening from the notification lands on Today
